      ...
      <div className="w-full max-w-sm mx-4 fade-in">
        <div className="glass-card p-8 space-y-6">
          <div className="text-center space-y-1">
            <h1 className="text-4xl font-bold gradient-text tracking-tight">Eka</h1>
          ...
          {isForgotPassword ? (
            resetSent ? (
              <div className="text-center space-y-4">
                ...
                <Button variant="ghost" onClick={() => { setIsForgotPassword(false); setResetSent(false); }} className="text-primary">
                  Back to login
                </Button>
              </div>
            ) : (
              <form onSubmit={handleForgotPassword} className="space-y-4">
                ...
                <Button type="button" variant="ghost" onClick={() => { setIsForgotPassword(false); setError(''); }} className="w-full text-muted-foreground">
                  Back to login
                </Button>
              </form>
            )
            ...
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              <button type="button" onClick={() => { setIsForgotPassword(true); setError(''); }} className="w-full text-sm text-muted-foreground hover:text-primary transition-colors">
                Forgot password?
              </button>
            </form>
          )}
