...
type AppRole = 'admin' | 'coo' | 'editor' | 'designer' | 'writer' | 'client' | 'camera_operator' | 'social_executive';
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const fetchProfileAndRole = useCallback(async (userId: string) => {
    try {
      const [profileRes, roleRes] = await Promise.all([
        (supabase.rpc as any)('get_own_profile').maybeSingle(),
  ...
  return (
    <AuthContext.Provider value={{ user, session, profile, role, loading, signIn, signOut, resetPassword }}>
      {children}
